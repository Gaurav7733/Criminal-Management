...
def login():
	if user_name.get()=="" or password.get()=="":
		...
		try:
			con = pymysql.connect(host="localhost",user="root",password="Gaurav",database="management")
			cur = con.cursor()
			...
			if row==None:
				...
				close()
				call(['python',r'E:\Projects Final\Final Project\main.py'])
			con.close()
...
def signup():
		...
		if first_name.get()=="" or last_name.get()=="" or age.get()=="" or city.get()=="" or add.get()=="" or user_name.get()=="" or password.get()=="" or very_pass.get()=="":
			...
			try:
				...
				if row!=None:
					...
					cur.execute("insert into user(first_name,last_name,age,gender,city,address,username,password) values(%s,%s,%s,%s,%s,%s,%s,%s)",
						(
						first_name.get(),
						last_name.get(),
						age.get(),
	...
	winsignup = Tk()
	winsignup.title("Criminal Management System")
	...
	sign_up_btn = Button(winsignup , text="Login" , command = switch )
...
# app title
win.title("Criminal Management System")

# window size
win.maxsize(width=500 ,  height=500)
win.minsize(width=500 ,  height=500)

SelfReg.bg=ImageTk.PhotoImage(file=r"E:\Projects Final\Final Project\images\ncr1 logo.jpg")
lbl_bg=Label(image=SelfReg.bg)
...
userentry = Entry(win, width=25 , textvariable = user_name)
userentry.focus()
userentry.place(x=200 , y=223)

passentry = Entry(win, width=25, show="*" ,textvariable = password)
